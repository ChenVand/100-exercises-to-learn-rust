...
impl Order {
    fn product_name_checker(product_name: &str) {
        assert!(product_name.len()>0 && product_name.len() <= 300, 
        "The product name can't be empty and it can't be longer than 300 bytes.");
    }

    fn quantity_checker(quantity: &u32) {
        assert!(*quantity>0 ,"The quantity must be strictly greater than zero.");
    }

    fn unit_price_checker(unit_price: &u32) {
        assert!(*unit_price>0, "The unit price is in cents and must be strictly greater than zero.");
    }
