...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let ticket_result: Result<Ticket, TicketNewError> = Ticket::new(title.clone(), description, status.clone());
    match ticket_result {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleEmpty(msg)) => {panic!("{}", msg);},
        Err(TicketNewError::TitleTooLong(msg)) => {panic!("{}", msg);},
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleEmpty("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleTooLong("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionEmpty("Description cannot be empty".to_string()));
        }
