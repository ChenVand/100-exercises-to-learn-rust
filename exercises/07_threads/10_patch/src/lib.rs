    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Update {
                ...
                let ticket_to_edit = store.get_mut(patch.id).unwrap();
                if let Some(title) = patch.title {ticket_to_edit.title = title};
                if let Some(description) = patch.description {ticket_to_edit.description = description};
                if let Some(status) = patch.status {ticket_to_edit.status = status};
